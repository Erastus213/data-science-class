...
def filter_data(df, department, location, remote):
    filtered_df = df[df["Department"].isin(department) & df["Office Location"].isin(location)]
    if remote != "All":
...
def display_metrics(filtered_df):
    ...
    with col1:
        st.metric("🙍‍♂️ Total Employee", len(filtered_df))

    with col2:
        avg_salary = filtered_df["Salary"].mean() if len(filtered_df) > 0 else 0 #tenary operator
...
def display_chart(filtered_df):
    if len(filtered_df) == 0:
        st.warning("No filter data to dsplay. Please adjust the data from the sidebar.")
        return
    ...
    with col4:
        ...
        location_count = filtered_df["Office Location"].value_counts()
        fig4 = px.bar(
